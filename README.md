...
- **Verified Switching**: Rather than firing the display switch at the same moment as the wake packet, it asks the TV whether it is actually up — over the network — and only then switches. A TV needs several seconds to power on and negotiate HDMI; switching into that gap is what makes a switch silently do nothing.
...
Every load is written to `debug.log` next to the executable, along with the outcome of each switch — whether the TV answered and on what, how long it took to come up, and whether it came up at all. If the TV does not wake, that log tells you whether the packets went out and whether the TV ever answered. A `config.json` that fails to parse is reported there too, rather than silently disabling the wake.
